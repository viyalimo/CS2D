...
class Server:
    def __init__(self, max_client, host):
        ...
        self.player_start_pos = ([45, 45, "L", 0], [45, 45, 'L', 0])
        self.player_data = {}
        self.members = []
        self.bullets = []
        self.qbullets = 0
    ...
    def connect(self, msg, conn, client_id):
        try:
            ...
            if self.qplayer == 1:
                ...
                self.player_data[client_id] = self.player_start_pos[0], [], self.HP
                self.mapa_x = msg[1]
                self.mapa_y = msg[2]
            ...
            if self.qplayer == 2:
                for member in self.members:
                    if member == client_id:
                        ...
                        self.player_data[client_id] = self.player_start_pos[1], [], self.HP
                        continue
        except Exception as e:
                        ...
                        for b in range(bul):
                            if b < bul:
                                ...
                                if pl_rect.collidepoint(bul_x, bul_y):
                                    ...
                                    self.bullets.pop(b)
                                    bul -= 1
                                    if self.player_data[i][2] > 0:
                                        print(self.player_data[i][2])
                                        self.player_data[i] = (self.player_data[i][0], self.player_data[i][1], self.player_data[i][2]-1)
                                    else:
                                        self.player_data[i][0][0] = 45
                                        self.player_data[i][0][1] = 45
                                        self.player_data[i][0][2] = 'L'
                                        self.player_data[i][0][3] = 0
                                        self.player_data[i] = (self.player_data[i][0], self.player_data[i][1], self.HP)
